...
// Use a static/dummy API key for now
const STATIC_API_KEY = "DUMMY_KEY";
console.log("Using API Key:", STATIC_API_KEY.slice(0, 10));

const app = express();
app.use(cors());
app.use(express.json());

// Initialize OpenAI with the dummy key
const openai = new OpenAI({
...
app.post("/analyze", async (req, res) => {
  try {
    ...
    const verdict =
      offerAmount < expectedLow
        ? "Undervalued"
        : offerAmount > expectedHigh
        ? "Overvalued"
        : "Fair";
...
Give a short (1–3 sentences) negotiation tip and a suggested counter-offer range (USD). Keep it actionable and concise.`;
    ...
    // Fallback suggestion (static)
    let suggestion = `Based on ${followers} followers and ${engagementRate}% engagement, a fair counter-offer is $${Math.round(offerAmount * 2)}.`;

    try {
      // Attempt AI call, but it will fail safely with dummy key
      const aiResponse = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You provide concise negotiation advice for influencer deals." },
      ...
      console.warn("OpenAI call skipped, using static suggestion.");
    }
